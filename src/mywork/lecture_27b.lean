...
begin
  assume inj,
  cases inj with r_tot r_one_to_one,
  unfold total_function at r_tot,
  unfold function,
  unfold single_valued,
  unfold inverse,
...
def bij_trans (s : β → γ → Prop)  (r : α → β → Prop) :
  ...
  begin
    ...
    cases s_tot with s_fun salldef,
    unfold function at s_fun,
    unfold single_valued at s_fun,
    unfold defined at salldef,
    unfold bijective,
    unfold surjective,
    unfold injective,
    unfold total_function,
    ...
    apply and.intro,
    apply and.intro,
    assume x y z esby esbz,
    cases esby with y qsxy,
    cases esbz with z qsxz,
    have rxy := and.elim_right qsxy,
    have rxz := and.elim_right qsxz,
    have yz := s_one_to_one rxy rxz
    exact yz,
